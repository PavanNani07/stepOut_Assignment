...
const AddTrain = ({ token }) => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await addTrain({ train_name: trainName, source, destination, total_seats: totalSeats, available_seats: totalSeats }, token);
    ...
    return (
        <form onSubmit={handleSubmit}>
            <input type="text" value={trainName} onChange={(e) => setTrainName(e.target.value)} placeholder="Train Name" required />
